write-host 'PowerShell Calculator for Montreal Cognitive Assessment'
write-host '======================================================='
[int]$V = Read-Host -Prompt 'Visuospatial/Executive: Please, type score for visuospatial abilities [0-5]'
while ($V -gt 5 -OR $V -lt 0) {
$V = Read-Host "Value is out of range. Please, enter a valid number"
}
[int]$N = Read-Host -Prompt 'Naming: Please, enter score for naming [0-3]'
while ($N -gt 3 -OR $N -lt 0) {
$N = Read-Host "Value is out of range. Please, enter a valid number"
}
...
[int]$moca = [int]$V + [int]$N + [int]$A1 + [int]$A2 + [int]$A3 + [int]$L1 + [int]$L2 + [int]$A4 + [int]$DR + [int]$O
Write-Host 'The total score of MoCA is' $moca '/30'
