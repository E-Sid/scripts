...
[int]$A0 = Read-Host -Prompt 'Please, re-enter value for orientation in time out of 4 to calculate M-ACE (day, date, month, year [0-4]'
...
while ($L4 -gt 1 -OR $L4 -lt 0) {
$L4 = Read-Host "value is out of range, please re-type the value"
}
[int]$L5 = Read-Host -Prompt 'Please, enter value for proverb-1 repetition [0-1]'
while ($L5 -gt 2 -OR $L5 -lt 0) {
$L5 = Read-Host "value is out of range, please re-type the value"
}
[int]$L6 = Read-Host -Prompt 'Please, enter value for object naming [0-12]'
while ($L6 -gt 12 -OR $L6 -lt 0) {
$L5 = Read-Host "value is out of range, please re-type the value"
}
...
[int]$sum = [int]$A1 + [int]$A2 + [int]$A3 + [int]$A4 + [int]$M1 + [int]$F1 + [int]$F2 + [int]$M2 + [int]$M3 + [int]$L1 + [int]$L2 + [int]$L3 + [int]$L4 + [int]$L5 + [int]$L6 + [int]$L7 + [int]$L8 + [int]$V1 + [int]$V2 + [int]$V3 + [int]$V4 + [int]$V5 + [int]$M4 + [int]$M5
