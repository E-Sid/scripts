write-host 'PowerShellCalculator for Addenbrooke's Cognitive Examination -R''
write-host '======================================================================'
...
[int]$A0 = Read-Host -Prompt 'Please, re-enter value for orientation in time [0-4] for M-ACE'
...
while ($M1 -gt 3 -OR $M1 -lt 0 -OR $M1 -gt $A3) {
...
[int]$L0 = Read-Host -Prompt 'Please, enter value for CLOSE YOUR EYES task [0-1]'
while ($L0 -gt 1 -OR $L0 -lt 0) {
$L0 = Read-Host "value is out of range, please, re-type the value"
}
...
[int]$L2 = Read-Host -Prompt 'Please, enter value for writing [0-1]'
...
[int]$L4 = Read-Host -Prompt 'Please, enter value for proverb-1 repetition [0-1]'
while ($L4 -gt 2 -OR $L4 -lt 0) {
$L4 = Read-Host "value is out of range, please re-type the value"
}
[int]$L5 = Read-Host -Prompt 'Please, enter value for proverb-2 repetition [0-1]'
while ($L5 -gt 2 -OR $L5 -lt 0) {
$L4 = Read-Host "value is out of range, please re-type the value"
}
[int]$L6A = Read-Host -Prompt 'Please, enter value for object naming [0-2]'
while ($6A -gt 2 -OR $L6A -lt 0) {
$L5A = Read-Host "value is out of range, please re-type the value"
}
[int]$L6 = Read-Host -Prompt 'Please, enter value for object naming [0-10]'
while ($L6 -gt 10 -OR $L6 -lt 0) {
$L5 = Read-Host "value is out of range, please re-type the value"
}
[int]$L7 = Read-Host -Prompt 'Please, enter value for comprehension [0-4]'
while ($L7 -gt 4 -OR $L7 -lt 0) {
$L7 = Read-Host "value is out of range, please re-type the value"
}
[int]$L8 = Read-Host -Prompt 'Please, enter value for reading [0-1]'
while ($L8 -gt 1 -OR $L8 -lt 0) {
$L7 = Read-Host "value is out of range, please re-type the value"
}
[int]$V1 = Read-Host -Prompt 'Please, enter value for intersecting intersecting pentagons [0-1]'
...
[int]$sum = [int]$A1 + [int]$A2 + [int]$A3 + [int]$A4 + [int]$M1 + [int]$F1 + [int]$F2 + [int]$M2 + [int]$M3 + [int]$L0 + [int]$L1 + [int]$L2 + [int]$L3 + [int]$L4 +  [int]$L6A + [int]$L5 + [int]$L6 + [int]$L7 + [int]$L8 + [int]$V1 + [int]$V2 + [int]$V3 + [int]$V4 + [int]$V5 + [int]$M4 + [int]$M5
Write-Host 'The total score of ACE-R is'$sum '/100'
[int]$mmse = [int]$A1 + [int]$A2 +[int]$A3 + [int]$A4 + [int]$M1 + [int]$L0 + [int]$L1 + [int]$L2 + [int]$L5 + [int]$L6A + [int]$V1 
Write-Host 'The MMSE score is' $mmse '/30'
[int]$mace = [int]$A0 + [int]$F2 + [int]$M2 + [int]$V3 + [int]$M4
Write-Host  'The M-ACE score is' $mace '/30'
...
[int]$sumL = [int]$L0 + [int]$L1 + [int]$L2 + [int]$L3 + [int]$L4 +  [int]$L5 + [int]$L6A + [int]$L6 + [int]$L7 + [int]$L8
